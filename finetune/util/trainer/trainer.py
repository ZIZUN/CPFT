...
class Trainer:
    def __init__(self, task, model,
                 train_dataloader: DataLoader, val_dataloader: DataLoader, test_dataloader: DataLoader = None,
        ...
        self.scheduler = get_cosine_schedule_with_warmup(
            self.optim,
            num_warmup_steps=0,
            num_training_steps=444444600)            
            # num_warmup_steps=100,
            # num_training_steps=1500)

        self.max_acc_model_path = ''
    ...
    def iteration(self, epoch, data_loader, train=True):
        ...
        for i, data in data_iter:
            ...
            output = self.model.forward(**data, mode='train')
        ...
        # eval
        eval_acc = self.evaluation(epoch, self.val_data, train=False)
        self.model.train
        # save
        ...
        if eval_acc > self.max_acc:
            if self.max_acc_model_path != '':
                os.remove(self.max_acc_model_path)
            self.max_acc = eval_acc
            self.max_acc_model_path = output_path
            self.count_for_stop = 0
            ...
            if self.count_for_stop == 10:
                self.model.load_state_dict(torch.load(self.max_acc_model_path))
                eval_acc = self.evaluation(epoch, self.test_data, train=False)
                print('===========Early Stopping!!===========')
                print('\nBest Model val_accuracy: '+ str(self.max_acc)[:10] + " test_accuracy: "+ "_"+str(eval_acc)[:10])
                print("Model name: output/"+ self.task +'/'+ str(self.now_iteration) +"_"+str(eval_acc)[:5] + '_' + str(self.seed) + "_" + self.model_name)
                logging.info('===========Early Stopping!!===========')
                logging.info('\nBest Model val_accuracy: '+ str(self.max_acc)[:10] + " test_accuracy: "+ "_"+str(eval_acc)[:10])
                logging.info("Model name: output/"+ self.task +'/'+ str(self.now_iteration) +"_"+str(eval_acc)[:5] + '_' + str(self.seed) + "_" + self.model_name)
                exit()  
    ...
    def evaluation(self, epoch, data_loader, train=False):
        ...
        with torch.no_grad():
            for i, data in data_iter:
                ...
                output = self.model.forward(**data, mode='test')
